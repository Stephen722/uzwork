...
function review_post(){
	$.ajax({
		url:path+"/uz-admin/project-mag",
		data:{
			process:"review_post",
			curProjectId: $("#curProjectId").val(),
		...
		success: function(data){
			...
			if(data.success) {
				popup_close();
				setTimeout("location.href = path + '/uz-admin/project-mag?process=review_list'", 2500);
			}
		},
...
<div id="reviewDIV" class="popup">
	<div class="head">
		<p class="title">项目审核</p>
		<p class="close"><i></i></p>
	</div>
	<input type="hidden" id="curStatusId" value="" />
	<input type="hidden" id="curProjectId" value="" />
